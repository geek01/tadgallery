...
myApp.onPageInit('cate', function(page) {
    ...
    $$('.password-modal' + csn).on('click', function() {
        ...
        var pass = $$(this).hasClass('check_pass');//一層換頁
        var check = sessionStorage.getItem('key' + ccsn);//二層換頁
        if (!check && !pass) {
            myApp.modalPassword('', '請輸入密碼', function(password) {
                $.post('pda.php', { op: 'check', csn: ccsn, passwd: password }, function(data) {
                    if (data.type == 'error') {
                        ...
                        $$('.album' + ccsn).addClass('check_pass');
                        sessionStorage.setItem('key' + ccsn, 'true');
